...
class Sales extends CI_Controller {
    ...
    function checkinvoicemanually(){
        $data = json_decode($this->input->raw_input_stream);
        $row_count = $this->db->query("SELECT * FROM  tbl_salesmaster WHERE SaleMaster_InvoiceNo=?",$data->invoiceNo)->num_rows();
        echo json_encode($row_count);
    }
    function purchasecheckinvoicemanually(){
         $data = json_decode($this->input->raw_input_stream);
        $row_count = $this->db->query("SELECT * FROM  tbl_purchasemaster WHERE PurchaseMaster_InvoiceNo=?",$data->invoiceNo)->num_rows();
        echo json_encode($row_count);
    }

    function checksmId(){
        $data = json_decode($this->input->raw_input_stream);
        $checkExist = $this->db->query("SELECT * FROM tbl_quotation_master WHERE sm_id=?",$data->smId)->num_rows();
    ...
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'pr_po_label'=>$data->sales->PRPO_label,
                'pr_po_text'=>$data->sales->PRPO_text
            );
            ...
            foreach($data->cart as $cartProduct){
                ...
                $checkOnCurrentStock = $this->db->query("SELECT * FROM tbl_currentinventory WHERE product_id=? and branch_id = ?",[$cartProduct->productId,$this->session->userdata('BRANCHid')])->num_rows();
                
              
                if($checkOnCurrentStock>0){
                //update stock
                $this->db->query("
            ...
            ", [$cartProduct->quantity, $cartProduct->productId, $this->session->userdata('BRANCHid')]);
                    ...
                    $result = $this->db->insert('tbl_currentinventory',$data);
        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != ''){
            ...
            $saleDetails = $this->db->query("
                select 
                    ...
                    pc.ProductCategory_Name,
                    u.Unit_Name,
                    (select b.brand_name from tbl_brand b where b.brand_SiNo=(select brand from tbl_product  where tbl_product.Product_SlNo=p.Product_SlNo)) as brand_name
                from tbl_saledetails sd
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
    ...
    public function getSaleDetailsForReturn(){
        ...
        $saleDetails = $this->db->query("
            ...
            join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
            join tbl_salesmaster sm on sm.SaleMaster_SlNo = sd.SaleMaster_IDNo
            left join tbl_brand b on p.brand = b.brand_SiNo 
    ...
    public function getSaleReturnDetails(){
        ...
        $returnDetails = $this->db->query("
            ...
            join tbl_brand b on p.brand = b.brand_SiNo
...
//    }
    ...
    // false sales
    public function falseSales($saleId) {
        $data['title'] = "False Sales Entry";
        $data['saleId'] = $saleId;
        $invoice = $this->db->query("select SaleMaster_InvoiceNo from tbl_salesmaster where SaleMaster_SlNo = ?", $saleId)->row();
        $data['invoice'] = $invoice->SaleMaster_InvoiceNo;
        $data['content'] = $this->load->view('Administrator/sales/false_sale', $data, true);
        $this->load->view('Administrator/index', $data);
    }

    public function falseSalesInvoice($saleId) {
        $data['title'] = "False Sales Invoice";
        $data['saleId'] = $saleId;
        $data['content'] = $this->load->view('Administrator/sales/false_sale_invoice', $data, true);
        $this->load->view('Administrator/index', $data);
    }

    public function falseSalesChallan($saleId) {
        $data['title'] = "False Challan";
        $data['saleId'] = $saleId;
        $data['content'] = $this->load->view('Administrator/sales/false_sale_challan', $data, true);
        $this->load->view('Administrator/index', $data);
    }

    public function addFalseSales() {
        $res = new stdClass;
        try {
            $this->db->trans_begin();
            $data = json_decode($this->input->raw_input_stream);
            ...
            $sale = array(
                'SaleMaster_InvoiceNo' => $data->sale->invoiceNo,
                'SaleMaster_customer_name' => $data->sale->customerName,
                'SaleMaster_customer_mobile' => $data->sale->customerMobile,
                'SaleMaster_customer_address' => $data->sale->customerAddress,
                'SaleMaster_TotalSaleAmount' => $data->sale->total,
                'SaleMaster_TotalDiscountAmount' => $data->sale->discount,
                'SaleMaster_TaxAmount' => $data->sale->vat,
                'SaleMaster_SubTotalAmount' => $data->sale->subTotal,
                'Status' => 'a',
                "AddBy" => $this->session->userdata("FullName"),
                'AddTime' => date("Y-m-d H:i:s"),
                'SaleMaster_branchid' => $this->session->userdata("BRANCHid"),
                'sale_id'=>$data->sale->saleId,
                'PRPO_label'=> isset($data->sale->PRPO_label) ? $data->sale->PRPO_label : '',
                'PRPO_text'=> isset($data->sale->PRPO_text) ? $data->sale->PRPO_text : ''
            );
            $this->db->insert('sale_masters', $sale);
            $saleId = $this->db->insert_id();
        
        
            foreach($data->cart as $cartProduct){
                $quotationDetails = array(
                    'SaleMaster_IDNo' => $saleId,
                ...
                $this->db->insert('sale_details', $quotationDetails);
            }
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $res->success = true;
                $res->message = 'Sales successfully';
                $res->saleId = $saleId;
            }
        } catch (\Exception $e) {
            $this->db->trans_rollback();
    ...
    public function upateFalseSales() {
        $res = new stdClass;
        try {
            $this->db->trans_begin();
            $data = json_decode($this->input->raw_input_stream);
            ...
            $this->db->where('id', $data->sale->id)->update('sale_masters', $sale);
            $saleId = $data->sale->id;
        
            // delete old sale detail data
            $this->db->where('SaleMaster_IDNo', $saleId)->delete('sale_details');

            // insert sale detail data
            foreach($data->cart as $cartProduct){
                $quotationDetails = array(
                    'SaleMaster_IDNo' => $saleId,
                    'product_name' => $cartProduct->name,
    ...
    public function getFalseSales() {
        $res = new stdClass;
        $data = json_decode($this->input->raw_input_stream);

        $clauses = "";

        if(isset($data->saleId) && $data->saleId != 0 && $data->saleId != ''){
            $clauses .= " and sm.sale_id = $data->saleId";
            $saleDetails = $this->db->query("
                select 
                    sd.*
                from sale_details sd
                join sale_masters sm on sm.id = sd.SaleMaster_IDNo
                where sm.sale_id = ?
            ", $data->saleId)->result();
