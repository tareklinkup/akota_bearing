...
class Products extends CI_Controller {
    ...
    public function get_brand_wise_product(){
        $brandReq = json_decode($this->input->raw_input_stream);
 ...
 public function addProduct() {
        ...
        try{
            ...
            $getProdName = trim($productObj->Product_Name," ") ;
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and brand=?", [$getProdName,$productObj->brand])->num_rows();
            if($productNameCount > 0){
                 $getPsl = $this->db->query("select * from tbl_product where Product_Name = ? and brand=?", [$getProdName,$productObj->brand])->row();


                $product['status'] = 'a';
                $this->db->where('Product_SlNo', $getPsl->Product_SlNo)->update('tbl_product', $product);
    ...
    public function updateProduct(){
        ...
        try{
            $productObj = json_decode($this->input->raw_input_stream);
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and Product_SlNo != ? and brand=?", [$productObj->Product_Name, $productObj->Product_SlNo,$productObj->brand])->num_rows();
            if($productNameCount > 0){
                $res = ['success'=>false, 'message'=>'Product name already exists'];
                echo json_encode($res);
                exit;
    ...
    public function getSearchingStock(){
        $data = json_decode($this->input->raw_input_stream);
        ...
        if(isset($data->branchId) && $data->branchId != '') {
            ...
            $filter .= " and brunch_id = $data->branchId";
        }
        ...
        if(isset($data->type) && $data->type != '' && $data->type == 'without') {
            $stockClause = " and current_quantity > 0";
            $filter .= " and current_quantity > 0";
        }
        ...
        //     $stock = array_map(function($brance) use($filter){
        //         $brance->brances = $this->db->query("
        //             select * from(
        //                 select
        //                     b.brunch_id,
        //                     b.Brunch_name,
        //                     (
        //                         select (cin.purchase_quantity + cin.sales_return_quantity + cin.transfer_to_quantity) - (cin.sales_quantity + cin.purchase_return_quantity + cin.damage_quantity + cin.transfer_from_quantity)
        //                     ) as current_quantity
        //                 from tbl_brunch b 
        //                 join tbl_currentinventory cin on b.brunch_id = cin.branch_id
        //                 where cin.product_id = ?
        //                 )as tbl
        //             where 1 = 1
        //             $filter
        ...
        $stock = array_map(function($branch) use($filter){
            $branch->brances = $this->db->query("
                select 
                    b.brunch_id,
                    b.Brunch_name,
                    (select ifnull(sum(pd.PurchaseDetails_TotalQuantity), 0) 
                            from tbl_purchasedetails pd 
                            where pd.Product_IDNo = $branch->Product_SlNo
                            and pd.PurchaseDetails_branchID = b.brunch_id
                    ...
                    (select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                            ...
                            where prd.PurchaseReturnDetailsProduct_SlNo = $branch->Product_SlNo
                            and prd.PurchaseReturnDetails_brachid = b.brunch_id) as purchase_returned_quantity,
                            
                    (select ifnull(sum(sd.SaleDetails_TotalQuantity), 0) 
                            from tbl_saledetails sd
                            where sd.Product_IDNo = $branch->Product_SlNo
                            and sd.SaleDetails_BranchId  = b.brunch_id
                            and sd.Status = 'a') as sold_quantity,
                            
                    (select ifnull(sum(srd.SaleReturnDetails_ReturnQuantity), 0)
                            from tbl_salereturndetails srd 
                            where srd.SaleReturnDetailsProduct_SlNo = $branch->Product_SlNo
                            and srd.SaleReturnDetails_brunchID = b.brunch_id) as sales_returned_quantity,
                    ...
                    (select ifnull(sum(dmd.DamageDetails_DamageQuantity), 0) 
                            ...
                            where dmd.Product_SlNo = $branch->Product_SlNo
                            and dmd.status = 'a'
                            and dm.Damage_brunchid = b.brunch_id) as damaged_quantity,
                
                    (select ifnull(sum(trd.quantity), 0)
                            from tbl_transferdetails trd
                            join tbl_transfermaster tm on tm.transfer_id = trd.transfer_id
                            where trd.product_id = $branch->Product_SlNo
                            and tm.transfer_from = b.brunch_id) as transfered_from_quantity,
    
                    (select ifnull(sum(trd.quantity), 0)
                            from tbl_transferdetails trd
                            join tbl_transfermaster tm on tm.transfer_id = trd.transfer_id
                            where trd.product_id = $branch->Product_SlNo
                            and tm.transfer_to = b.brunch_id) as transfered_to_quantity,
    ...
    public function getCurrentStock(){
        ...
        if(isset($data->productId) && $data->productId > 0){
            $prodclauses .= " and product_id='$data->productId'";
        }
        $stock = $this->db->query("
            select * from(
                select
                    ci.*,
                    (select sdd.AddTime  from tbl_saledetails sdd where ci.product_id=sdd.Product_IDNo order by SaleDetails_SlNo DESC LIMIT 1) as getsaleupdate,
                    (select pdd.AddTime  from  tbl_purchasedetails pdd where ci.product_id=pdd.Product_IDNo order by PurchaseDetails_SlNo DESC LIMIT 1) as getpurchaseupdate,
                    (select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity) - (ci.sales_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
