...
class Page extends CI_Controller {
    ...
    public function imageUpload($file_name_get){
		...
		move_uploaded_file($file_temp,'uploads/branch_thum/'.$new_name);
		return $new_name;
	}
    ...
    public function updateBranch()  {
        ...
        try{
            ...
            $oldImage = $this->db->query("select * from tbl_brunch where brunch_id =?", $branch->branchId)->row();
            $image="";
            if (isset($_FILES['image'])) { 
                $image = $this->imageUpload($_FILES['image']);

                if($oldImage->image != null) {
                    $img_unlink = 'uploads/branch_thum/'. $oldImage->image;
                    unlink($img_unlink);
                }
            }else{
    ...
    public function insert_brand(){
		...
		$branch = $this->brunch;
        $query = $this->db->query("SELECT brand_name from tbl_brand where brand_branchid = '$branch' AND brand_name = '$brandname' AND status='a'");
