...
class Customer extends CI_Controller
{
    ...
    public function getCustomerDue(){
        ...
        $dueResult = $this->db->query("
            ...
            from tbl_customer c
            where c.status = 'a'
            and c.Customer_brunchid = '$branchId' 
            and c.Customer_Type != 'G'
            $customerIdClause
        ")->result();
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            $customerMobileCount = $this->db->query("select * from tbl_customer where Customer_Mobile = ? and status = 'a'", $customerObj->Customer_Mobile)->num_rows();
    ...
    public function updateCustomer()
    {
        ...
        try{
            $customerObj = json_decode($this->input->post('data'));
            $customerMobileCount = $this->db->query("select * from tbl_customer where Customer_Mobile = ? and Customer_SlNo != ? and status = 'a'", [$customerObj->Customer_Mobile, $customerObj->Customer_SlNo])->num_rows();
            if($customerMobileCount > 0){
                $res = ['success'=>false, 'message'=>'Mobile number already exists'];
                echo Json_encode($res);
                exit;
