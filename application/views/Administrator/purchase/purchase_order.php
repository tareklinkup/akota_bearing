	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			<div class="form-group">
				...
				<div class="col-sm-2"> 
					<input type="text" id="invoice" name="invoice" :disabled="purchase.purchaseId!=0"  v-model="purchase.invoice"  style="height:26px;"/>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					<input class="form-control" id="purchaseDate" name="purchaseDate" type="date" v-model="purchase.purchaseDate"  style="border-radius: 5px 0px 0px 5px !important;padding: 4px 6px 4px !important;width: 230px;" />
				</div>
			</div>
		</div>
	</div>
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-sm-7">
										<v-select v-bind:options="brands" v-model="selectedBrand" id="brandSelect" label="brand_name" v-on:input="brandOnChange"></v-select>
									</div>
									<div class="col-sm-1" style="padding: 0;">
								...
								<div class="form-group">
									...
									<div class="col-sm-3">
										<input type="text" id="purchaseRate" name="purchaseRate" ref="Product_Purchase_Rate" class="form-control" placeholder="Pur. Rate" v-model="selectedProduct.Product_Purchase_Rate" />
									</div>

									<label class="col-sm-2 control-label no-padding-right"> Quantity </label>
									<div class="col-sm-3">
										<input type="text" step="0.01" id="quantity" name="quantity" class="form-control" placeholder="Quantity"  v-model="selectedProduct.quantity" v-on:input="productTotal" required/>
									</div>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-4 control-label no-padding-right" style="text-align: right;color: green;font-weight: bold;font-size: 14px"> Stock : {{avaiStock}} </label>
									<div class="col-sm-8">
	...
	new Vue({
		...
		methods:{
			...
			async onChangeProduct(){
				this.$refs.Product_Purchase_Rate.focus(); 
				this.selectedProduct.Product_Purchase_Rate = ''
				await axios.post('/get_product_stock',{productId:this.selectedProduct.Product_SlNo})
				.then(res=>{
					this.avaiStock = res.data;
				})
			},
			productTotal(){
			...
			async savePurchase(){
				...
				if(this.purchase.purchaseId != 0){
					...
					let invoiceStatus = await axios.post('/purchasecheckinvoicemanually',{invoiceNo:this.purchase.invoice})
				.then(res=>{ 
						return res.data;
				})
				if (invoiceStatus>0) {
			...
			getPurchase(){
				axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					this.selectedSupplier.display_name = purchase.Supplier_Type == 'G' ? 'Local Supplier' : `${purchase.Supplier_Code} - ${purchase.Supplier_Name}`;
