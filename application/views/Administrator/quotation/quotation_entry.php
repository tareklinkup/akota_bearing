...
<div id="quotation" class="row">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			<div class="form-group">
				...
				<div class="col-sm-2">
					<input type="text" class="form-control" v-model="quotation.invoiceNo" />
				</div>
			</div>
			<div class="form-group">
	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		...
		<div class="form-group">
				...
				<div class="col-sm-2">
					<input type="text" id="PRPO_label" class="form-control" v-model="quotation.PRPO_label"  />
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> Challan  No </label>
				<div class="col-sm-2">
					<input type="text" id="PRPO_text" class="form-control" v-model="quotation.PRPO_text"  />
				</div>
			</div>
		</div>
		</div>
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="form-group clearfix">
								...
								<div class="col-sm-8">
									<input type="text" style="color: #222" class="form-control" v-model="quotation.customerName" placeholder="Customer Name">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input type="text" style="color: #222" placeholder="Mobile No" class="form-control" v-model="quotation.customerMobile" />
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<textarea style="color: #222" placeholder="Address" class="form-control" v-model="quotation.customerAddress"></textarea>
								</div>
							</div>
						</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ product.total }}</td> 
							<td><a style="margin-right: 5px" href="" v-on:click.prevent="edit(product,sl)"><i class="fa fa-edit"></i></a><a href="" v-on:click.prevent="removeFromCart(sl)"><i class="fa fa-trash"></i></a></td>
						</tr>
					</tbody>
				</table>
			</div>
...
<?php
	$last_id = $this->db->query("SELECT SaleMaster_InvoiceNo FROM tbl_salesmaster WHERE  SaleMaster_SlNo=? ",$quotationId)->row();
	$getInv = empty($last_id->SaleMaster_InvoiceNo)?0:$last_id->SaleMaster_InvoiceNo;
 ?>
<script>
	...
	new Vue({
		...
		methods:{
			async checksmId(){
				await axios.post('/check_sm_id',{smId:this.smId}).then(res=>{
					this.quotation.action = res.data;
				})
				if (this.quotation.action==0) {
					this.getSales();
				}else{
			...
			addToCart(){
				...
				if (this.cartAction=='edit') {
                   this.cart[this.selectedProduct.cartId].name = this.selectedProduct.Product_Name;
                   this.cart[this.selectedProduct.cartId].Product_SellingPrice = this.selectedProduct.Product_SellingPrice;
                   this.cart[this.selectedProduct.cartId].quantity = this.selectedProduct.quantity;
                   this.cart[this.selectedProduct.cartId].total = this.selectedProduct.total;
			...
			edit(product,sl){
				this.cartAction = 'edit';
				this.selectedProduct.Product_Name = product.name
				this.selectedProduct.Product_SellingPrice = product.Product_SellingPrice
				this.selectedProduct.quantity = product.quantity
				this.selectedProduct.total = product.total
				this.selectedProduct.cartId  = sl
			...
			saveQuotation(){
				...
				if(this.quotation.invoiceNo == '') {
					alert('Invoice is required');
					return;
				}
				let url = "/update_quotation";
				...
				axios.post(url, data).then(async res=> {
					...
					if(r.success){
						...
						if(conf){
							...
							window.location = '/quotation/'+r.quotationId;
						} else {
							window.location = '/quotation/'+r.quotationId;
						}
					}
			...
			getQuotations(){
				axios.post('/get_quotations', {quotationId: this.quotation.quotationId}).then(res=>{
					...
					this.quotation.PRPO_label = quotation.PRPO_label;
					this.quotation.PRPO_text = quotation.PRPO_text;
			...
			getSales(){
				axios.post('/get_sales', {salesId: this.smId}).then(res=>{
					let r = res.data;
					let sales = r.sales[0];
					this.quotation.salesFrom = sales.SaleMaster_branchid;
					this.quotation.salesDate = sales.SaleMaster_SaleDate;
					this.quotation.salesType = sales.SaleMaster_SaleType;
					this.quotation.customerName = sales.Customer_Name;
					this.customerMobile = sales.Customer_Mobile;
					this.customerAddress = sales.Customer_Address;
					this.quotation.subTotal = sales.SaleMaster_SubTotalAmount;
					this.quotation.discount = sales.SaleMaster_TotalDiscountAmount;
					this.quotation.PRPO_label = sales.PRPO_label;
					this.quotation.PRPO_text = sales.PRPO_text;
					this.quotation.vat = sales.SaleMaster_TaxAmount;
					this.quotation.total = sales.SaleMaster_TotalSaleAmount;
					this.vatPercent = parseFloat(this.quotation.vat) * 100 / parseFloat(this.quotation.subTotal);
					this.discountPercent = parseFloat(this.quotation.discount) * 100 / parseFloat(this.quotation.subTotal);
						this.quotation.customerName = sales.Customer_Name
						this.quotation.customerMobile = sales.Customer_Mobile
						this.quotation.customerAddress = sales.Customer_Address
