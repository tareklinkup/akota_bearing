...
<div class="row" id="supplierPaymentReport">
	...
	<div class="col-sm-12" style="display:none;" v-bind:style="{display: showTable ? '' : 'none'}">
		...
		<a href="javascript:" 
			onclick="exportToExcel('supplier-payment', 'Supplier Payment - <?php echo date('D d F, Y') ?>')"
		> 
			<i class="fa fa-file-excel-o"></i> Supplier Payment Excel 
		</a> | &nbsp;

		<a href="javascript:" 
			onclick="exportToWord('Supplier Payment - <?php echo date('D d F, Y') ?>')"
		> 
			<i class="fa fa-file-word-o"></i> Supplier Payment Word 
		</a>

		<div class="table-responsive googoose-wrapper" id="reportTable">
			<table class="table table-bordered" id="supplier-payment">
				<thead>
					<tr>
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align: right;"><strong>{{ payments.reduce((p, c) => { return +p + +c.cash_received }, 0).toFixed(2) }}</strong></td>
						<td></td>
					</tr>
				</tbody>
		...
		methods: {
			...
			getReport() {
				...
				axios.post('/get_supplier_ledger', data).then(res => {
					...
					this.payments = Object.values(res.data.payments);
