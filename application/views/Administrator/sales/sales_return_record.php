				...
				<table class="record-table">
					...
					<tbody>
						<tr v-for="details in returnDetails">
							...
							<td style="text-align:center">{{ details.brand_name }}</td>
							<td style="text-align:center"> {{ details.origin }}</td>
							...
							<td class="text-center">
								<?php if($this->session->userdata('accountType') != 'u'){?>
									<a href="" title="Delete Return" @click.prevent="deleteReturn(details.SaleReturn_IdNo)"><i class="fa fa-trash"></i></a>
								<?php }?>
							</td>
						</tr>
					</tbody>
	...
	new Vue({
		...
		methods: {
			...
			async deleteReturn(id) {
				...
				await axios.post('/delete_sale_return', { id: id })
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success) {
						this.getSaleReturnDetails();
					}
				})
			},
			async print(){
