	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			<div class="form-group">
				...
				<div class="col-sm-2"> 
					<input type="text" id="invoiceNo" :disabled=" sales.salesId != 0"    v-model="sales.invoiceNo"  />
				</div>
			</div>
			<div class="form-group">
		...
		<div class="form-group">
				...
				<div class="col-sm-2">
					<input type="text" id="PRPO_label" class="form-control" v-model="sales.PRPO_label"  />
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> Challan  No </label>
				<div class="col-sm-2">
					<input type="text" id="PRPO_text" class="form-control" v-model="sales.PRPO_text"  />
				</div>
			</div>
		</div>
		</div>
				...
				<div class="widget-main">
					...
					<div class="row">
						<div class="col-sm-5">
							...
							<div class="form-group">
								<label class="col-sm-4 control-label no-padding-right"> Parent </label>
								<div class="col-sm-7">
									<v-select v-bind:options="parents" label="display_name" v-model="selectedCustomer" v-on:input="customerOnChange"></v-select>
								</div>
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<v-select v-bind:options="brands" role="combobox" id="brandSelect" v-model="selectedBrand"  label="brand_name" v-on:input="brandOnChange"></v-select>
									</div>
									<div class="col-sm-1" style="padding: 0;">
										<a href="<?= base_url('brand')?>" class="btn btn-xs btn-danger" style="height: 25px; border: 0; width: 27px; margin-left: -10px;" target="_blank" title="Add New brand"><i class="fa fa-plus" aria-hidden="true" style="margin-top: 5px;"></i></a>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="number" id="salesRate" placeholder="Rate" ref="quantity" step="0.01" class="form-control" v-model="selectedProduct.Product_SellingPrice" v-on:input="productTotal"/>
									</div>
								</div>
								<div class="form-group">
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:15%;color:#000;">Origin</th>
	...
	new Vue({
		...
		watch: {
			...
			async selectedChildren(child) {
				...
				this.selectedCustomer.Customer_Mobile = child.Customer_Mobile;
				this.selectedCustomer.Customer_Address = child.Customer_Address;

				await axios.post('/get_customer_due',{customerId: child.Customer_SlNo}).then(res=>{
		...
		methods:{
			async brandOnChange(){
				if (this.selectedBrand.brand_SiNo !=0) {
					await axios.post('/get_brand_wise_product',{brand_sl_no:this.selectedBrand.brand_SiNo})
				.then(res=>{
			...
			async productOnChange(){
				...
				this.selectedProduct.Product_SellingPrice = ''
			},
			...
			addToCart(){
				...
				//this.clearProduct();
				this.calculateTotal();
				let productSearchBox = document.querySelector('#brandSelect input[role="combobox"]');
			...
			async saveSales(){
				...
				if(this.selectedCustomer.Customer_SlNo == '' && this.selectedChildren == null){
				...
				if(this.sales.salesId != 0){
					...
					let invoiceStatus = await axios.post('/checkinvoicemanually',{invoiceNo:this.sales.invoiceNo})
				.then(res=>{ 
						return res.data;
				})
				if (invoiceStatus>0) {
				...
				this.sales.customerId = this.selectedChildren == null ? this.selectedCustomer.Customer_SlNo : this.selectedChildren.Customer_SlNo;
			...
			getSales(){
				axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					this.sales.PRPO_label = sales.pr_po_label;
					this.sales.PRPO_text = sales.pr_po_text;
					...
					this.selectedCustomer = {
						...
						display_name: sales.Customer_Type == 'G' ? 'Local Customer' : `${sales.Customer_Code} - ${sales.Customer_Name}`,
