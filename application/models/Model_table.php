...
class Model_Table extends CI_Model{
    ...
    public function generateQuotationInvoice(){
		$invoice = 'INV-' . date('Y') . "00001";
        $year = date('Y');
        $quotations = $this->db->query("select * from tbl_quotation_master qm where qm.SaleMaster_InvoiceNo like 'INV-$year%'");
        if($quotations->num_rows() != 0){
            $newQuotationId = $quotations->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000');
            $invoice = 'INV-' . date('Y') . (strlen($newQuotationId) > count($zeros) ? $newQuotationId : $zeros[count($zeros) - strlen($newQuotationId)] . $newQuotationId);
		}
